...
![Snort](https://img.shields.io/badge/-Snort_IDS-FF6B6B?&style=for-the-badge&logo=SecurityScorecard&logoColor=white)

**Enterprise-Grade Security Monitoring & Threat Detection Platform**
...
The goal of this project is to design and implement a **scalable Splunk Cluster Architecture** hosted on AWS for centralized log management and real-time security monitoring. This system ingests and processes network security alerts from **Snort IDS** and **DHCP logs** from a Wi-Fi router. By leveraging the power of AWS infrastructure, this setup provides robust and fault-tolerant data collection, analysis, and visualization capabilities, enhancing threat detection and network performance monitoring.

&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; ![Screencastfrom2025-03-1105-54-01-ezgif com-speed](https://github.com/user-attachments/assets/5717a7a4-cf31-496b-9ffb-bc7d26e631fe)  
&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; **REF 1 - SPLUNK CLUSTERED ARCHITECTURE**

This project provides a comprehensive understanding of both security monitoring and cloud-based architecture, allowing for the development of a flexible and scalable solution for real-time network analysis and threat detection.
...
|--------------|-----------|-------------|
| **SIEM Platform** | Splunk Enterprise | Clustered architecture, log ingestion, parsing, and visualization |
| **IDS/IPS** | Snort IDS | Network intrusion detection alerts generation |
| **Network Equipment** | Wi-Fi Router | DHCP logs collection and processing |
| **Operating System** | Linux/Ubuntu | Splunk components configuration |
| **Integration** | Splunk TAs | Simplifying integration of Snort and DHCP logs |
| **Cloud Platform** | Amazon Web Services (AWS) | Scalable compute resources and infrastructure management |
...
graph TB
    subgraph "Local Environment"
        HF[Heavy Forwarder<br/>Ubuntu Local Machine]
        SNORT[Snort IDS<br/>Alert Generation]
        ROUTER[Wi-Fi Router<br/>DHCP Logs]
    end
    
    subgraph "AWS Cloud Infrastructure"
        subgraph "Indexer Cluster"
            IDX1[Indexer 1<br/>Data Storage & Processing]
            IDX2[Indexer 2<br/>Data Storage & Processing]
            CM[Cluster Master<br/>Coordination & Management]
        end
        
        subgraph "Search Head Cluster"
            SH1[Search Head 1<br/>Search Interface]
            SH2[Search Head 2<br/>Search Interface]
        end
    end
...
**REF 4 - Successful Splunk Installation**
...
#### 🛡️ Installing and Configuring Snort
...
### ☁️ Setting Up AWS EC2 Instances and Splunk Cluster Architecture

#### 🏗️ Setting Up the indexer cluster on AWS EC2 instances

- To set up an indexer cluster, we atleast need 2 indexers and 1 cluster master (manager node)
- Let's spin up 3 EC2 instances with the below configuration:

**Instance Configuration:**
```
Operating System: Ubuntu
Instance Type: t2.medium
Security Group: Keep the same security group you select for all instances
...
- Select the instance **"Indexer_1"** and click **Associate**  
...
- Let's spin up 2 more EC2 instances with the below configuration:
...
- SSH into **"SearchHead_1"** and **"SearchHead_2"** and follow the same installation steps as before
- Now let's note down the IP addresses of all our AWS Instances including search heads:
...
- Enable SSL (HTTPS) on the above instances
- Log in to splunk on each of the instance and display their instance names on each of them respectively through global banners
...
Here are the configuration that we have copied for both the indexes:

```
[dhcp_logs]
coldPath = $SPLUNK_DB/dhcp_logs/colddb
homePath = $SPLUNK_DB/dhcp_logs/db
maxTotalDataSizeMB = 512000
thawedPath = $SPLUNK_DB/dhcp_logs/thaweddb

[snort_alert]
coldPath = $SPLUNK_DB/snort_alert/colddb
homePath = $SPLUNK_DB/snort_alert/db
maxTotalDataSizeMB = 512000
thawedPath = $SPLUNK_DB/snort_alert/thaweddb
```
...
![Screenshot from 2025-03-10 21-04-56](https://github.com/user-attachments/assets/83c7d4de-3ba7-4cae-aad6-2df48b64e456)  
...
![Screenshot from 2025-03-10 21-10-54](https://github.com/user-attachments/assets/67f987fc-527a-4b94-a574-b77f59740135)  
...
![Screenshot from 2025-03-10 21-12-28](https://github.com/user-attachments/assets/6b569b65-4c9d-4045-897b-dc639b7b3212)  
...
![Screenshot from 2025-03-10 21-15-11](https://github.com/user-attachments/assets/4a1ef1e6-dc5b-4420-b3f2-580395c58363)  
...
sudo ./splunk apply cluster-bundle
```
...
![Screenshot from 2025-03-10 21-42-36](https://github.com/user-attachments/assets/80b732e9-705d-49e7-bdac-090e75f1b6bc)  
**REF 51 - Clicking on Add New to Configure Forwarding**
...
![Screenshot from 2025-03-10 21-53-34](https://github.com/user-attachments/assets/15d05595-d2bd-46d3-a3e9-9f57a0750fbb)  
...
![image (3)](https://github.com/user-attachments/assets/ef9d6ca2-7e79-4fb1-be36-4756a25f4233)  
...
**Purpose:** This dashboard can be used to monitor network scanning and ping activities. Frequent ping events, especially those involving external IP addresses, could be signs of probing or network reconnaissance.

![Screenshot from 2025-03-12 00-20-20](https://github.com/user-attachments/assets/2477fb14-5847-4852-ac86-6a06817f9197)

**Key Visualizations:**

#### 📈 Timechart of Ping Detected Alerts
A line chart showing the number of Ping-related alerts on my IP Address (e.g., Ping Detected!) over time. This can help identify patterns of network activity or potential reconnaissance attacks.

**Search Command Used:**
```spl
index="snort_alert" source="/var/log/snort/alert" 
| where dst_ip="192.168.0.5" 
| timechart count by Number_of_Pings
```

#### 🏆 Top Source IPs for Ping Events
A bar chart showing the most frequent source IP addresses that trigger Ping alerts on my IP Address. This could highlight potential unauthorized scanning or reconnaissance.

**Search Command Used:**
```spl
index="snort_alert" source="/var/log/snort/alert"
| where dst_ip="192.168.0.105" 
| chart count by src_ip
```

#### 🎯 Top Destination IPs for Ping Events
A bar chart or table showing which destination IPs are being targeted by Pings, helping identify critical systems being probed.

**Search Command Used:**
```spl
index="snort_alert" source="/var/log/snort/alert" 
| chart count by dst_ip
```
...
|-----------------|-----------------|
| ✅ **Multi-node Splunk Cluster** | Successfully deployed a distributed Splunk architecture on AWS |
| ✅ **Real-time Threat Detection** | Implemented live monitoring using Snort IDS integration |
| ✅ **Comprehensive Dashboards** | Created security monitoring visualizations and analytics |
| ✅ **High Availability** | Achieved fault tolerance through cluster replication |
| ✅ **Performance Optimization** | Optimized for high-volume log ingestion and processing |
| ✅ **Cloud Integration** | Leveraged AWS infrastructure for scalability and reliability |
...
*Let's connect and discuss cybersecurity, cloud architecture, and security monitoring!*
...
**🛡️ "In cybersecurity, preparation and vigilance are your strongest defenses"**
